...
import { 
    ...
    SET_STATUS, SET_INPUT, SET_MESSAGE

} from './constants';

import { getData, saveData, detailData, editData } from '../../api/absensiAPI';
...
export const fetchData = () => {
    return  async (dispatch, getState) => {
        ...
        let perPage = getState().absensies.perPage || '';
        let currentPage = getState().absensies.currentPage || 1;
        let keyword = getState().absensies.keyword || '';
...
export const storeData = () => {
    return async (dispatch, getState) => {
        try {
            let token = getState().auth.token || '';
            let data = getState().absensies.input || [];
